...
void settings() { size(int(displayWidth * 0.5), int(displayHeight * 0.5)); }
int WinHeight=int(displayHeight*0.5);
int WinWideth=int(displayWidth*0.5);

void setup() {
surface.setTitle("Crop Demo - Command System (Single File)");
app = new App();
}
...
/* 五大模块
    ...
    Tool System：负责处理工具选择，记录连续操作的结果。将结果发送给CommandManager
...
void onKeyPressed(char k) {
    ...
    if (ctrl && (k=='z' || k=='Z')) { history.undo(doc); return; }
    if (ctrl && (k=='y' || k=='Y')) { history.redo(doc); return; }

    if (k=='o' || k=='O') { ui.openFileDialog(); return; }
    if (k=='m' || k=='M') { tools.setTool(new MoveTool()); return; }
    if (k=='c' || k=='C') { tools.setTool(new CropTool(history)); return; }
}
}
...
int w, h;
CanvasSpec(int w, int h) { this.w = w; this.h = h; }
}
...
float screenToCanvasX(float sx) { return (sx - panX) / zoom; }
float screenToCanvasY(float sy) { return (sy - panY) / zoom; }

void zoomAroundMouse(float delta) {
    float oldZoom = zoom;
    float factor = pow(1.08, -delta);
...
// =======================================================
// 4) ToolSystem: continuous interaction + overlay
// =======================================================
interface Tool {
...
void setTool(Tool t) { active = t; }

void mousePressed(Document doc, int mx, int my, int btn)  { if (active != null) active.mousePressed(doc, mx, my, btn); }
void mouseDragged(Document doc, int mx, int my, int btn)  { if (active != null) active.mouseDragged(doc, mx, my, btn); }
void mouseReleased(Document doc, int mx, int my, int btn) { if (active != null) active.mouseReleased(doc, mx, my, btn); }
void mouseWheel(Document doc, float delta)                { if (active != null) active.mouseWheel(doc, delta); }
void drawOverlay(Document doc)                            { if (active != null) active.drawOverlay(doc); }

String activeName() { return (active == null) ? "None" : active.name(); }
}
...
class CropTool implements Tool {
    ...
    boolean dragging = false;
float sx, sy, cx, cy; // in canvas coords

    CropTool(CommandManager history) { this.history = history; }
    ...
    dragging = true;
    sx = doc.view.screenToCanvasX(mx);
    sy = doc.view.screenToCanvasY(my);
    cx = sx; cy = sy;
}

public void mouseDragged(Document doc, int mx, int my, int btn) {
    if (!dragging) return;
    cx = doc.view.screenToCanvasX(mx);
    cy = doc.view.screenToCanvasY(my);
}
...
public void mouseReleased(Document doc, int mx, int my, int btn) {
    ...
    IntRect r = buildClampedRect(doc, sx, sy, cx, cy);
...
public void drawOverlay(Document doc) {
    ...
    float x1 = min(sx, cx), y1 = min(sy, cy);
    float x2 = max(sx, cx), y2 = max(sy, cy);
...
// =======================================================
// 5) CommandSystem: discrete actions + Undo/Redo
// =======================================================
interface Command {
...
CropCommand(IntRect rect) { this.rect = rect; }
...
UI() {
    ...
    btnOpen = new UIButton(x, y, w, h, "Open (O)"); y += h + gap;
    btnMove = new UIButton(x, y, w, h, "Move (M)"); y += h + gap;
    btnCrop = new UIButton(x, y, w, h, "Crop (C)"); y += h + gap;
    btnUndo = new UIButton(x, y, w, h, "Undo");      y += h + gap;
    btnRedo = new UIButton(x, y, w, h, "Redo");      y += h + gap;
}
...
boolean handleMousePressed(App app, int mx, int my, int btn) {
    ...
    if (btnOpen.hit(mx, my)) { openFileDialog(); return true; }
    if (btnMove.hit(mx, my)) { app.tools.setTool(new MoveTool()); return true; }
    if (btnCrop.hit(mx, my)) { app.tools.setTool(new CropTool(app.history)); return true; }
    if (btnUndo.hit(mx, my)) { app.history.undo(app.doc); return true; }
    if (btnRedo.hit(mx, my)) { app.history.redo(app.doc); return true; }

    return true; // consume clicks on panel
}

boolean handleMouseDragged(App app, int mx, int my, int btn) { return (mx >= panelX); }
boolean handleMouseReleased(App app, int mx, int my, int btn) { return (mx >= panelX); }
boolean handleMouseWheel(App app, float delta) { return false; }
